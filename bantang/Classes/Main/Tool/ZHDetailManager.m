...
#import "ZHDetailManager.h"
#import "FMDB.h"
@interface ZHDetailManager (){
    //    专门用来操作数据库的类
    FMDatabase *_fmdb;
    NSLock *_lock;
}
...
@end
@implementation ZHDetailManager
...
+(instancetype)defaultManager{
    static ZHDetailManager *dm = nil;
    @synchronized(self){
        if (!dm) {
            dm = [ZHDetailManager new];
        }
    }
    return dm;
}
-(void)insertDataWithModel:(ZHBaseModel *)bm{
    ...
    BOOL isInsert = [_fmdb executeUpdate:insertSql,bm.id,bm.title,bm.price,bm.image];
    if (!isInsert) {
...
-(void)updateModel:(ZHBaseModel *)dm forIndex:(NSInteger)index{
    ...
    BOOL isUpdate =  [_fmdb executeUpdate:updateSql,dm.id,dm.title,dm.price,dm.image,[NSNumber numberWithInteger:index]];
    if (!isUpdate) {
