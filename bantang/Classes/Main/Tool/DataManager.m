...
- (instancetype)init{
    ...
    if (self) {
//       数据库路径(必须保证文件夹是存在的)
        NSString *path = [NSHomeDirectory() stringByAppendingPathComponent:@"Documents/123.db"];
//        初始化操作数据库的对象
...
-(void)insertDataWithModel:(ZHBaseModel *)bm{
    ZHListModel *listModel = (id)bm;
//    插入sql语句
    NSString *insertSql = @"insert into userInfo(idStr,title,price,image) values(?,?,?,?)";
//    执行sql语句,并补全参数,(参数必须为对象类型)
    BOOL isInsert = [_fmdb executeUpdate:insertSql,bm.id,listModel.author.nickname,bm.price,bm.image];
    if (!isInsert) {
...
-(void)updateModel:(ZHBaseModel *)dm forIndex:(NSInteger)index{
    ZHListModel *listModel = (id)dm;
    NSString *updateSql = @"update userInfo set idStr=?,title=?,price=?,image=? where id=?";
    BOOL isUpdate =  [_fmdb executeUpdate:updateSql,dm.id,listModel.author.nickname,dm.price,dm.image,[NSNumber numberWithInteger:index]];
    if (!isUpdate) {
...
//        遍历查询结果
        ZHBaseModel *dm = [ZHBaseModel new];
